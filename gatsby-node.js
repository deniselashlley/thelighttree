  ...
  `).then(result => {
    ...
    const postOrPage = result.data.allMarkdownRemark.edges.filter(edge => {
      if (edge.node.frontmatter.templateKey === "navbar") {
        ...
        return false;
      } else if (edge.node.frontmatter.templateKey === "introduction")  {
        return false;
      } else if (edge.node.frontmatter.templateKey === "schedule")  {
        return false;
      } else if (edge.node.frontmatter.templateKey === "my-story")  {
        return false;
      } else if (edge.node.frontmatter.templateKey === "contact")  {
        return false;
      } else if (edge.node.frontmatter.templateKey === "testimonials")  {
          return false;
      } else if (edge.node.frontmatter.templateKey === "services")  {
          return false;
      } else {
